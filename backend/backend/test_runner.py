...
class TimedTextTestResult(TextTestResult):
    ...
    def addSuccess(self, test, end_time=None):
        ...
        self.durations[test] = (end_time - self._starts[test]) * 1000
...
class MiliSecondTimedTestRunner(DiscoverRunner):
...
def generate_report(durations, limit=NUM_SLOWEST_TESTS, full_report=False):
    ...
    if full_report:
        reports.append(
            tabulate(module_report_data, headers=["Module", "Duration (ms)"], tablefmt="github",  floatfmt=".1f")
        )
        reports.append(
            tabulate(class_report_data, headers=["Class", "Duration (ms)"], tablefmt="github",  floatfmt=".1f")
        )

    reports.append(tabulate(method_report_data, headers=["Test", "Duration (ms)"], tablefmt="github",  floatfmt=".1f"))
